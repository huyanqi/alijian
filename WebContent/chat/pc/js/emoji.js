...
(function(local_setup) {
	...
	/**
	 * Configuration details for different image sets. This includes a path to a
	 * directory containing the individual images (`path`) and a URL to sprite
	 * sheets (`sheet`). All of these images can be found in the [emoji-data
	 * repository]{@link https://github.com/iamcal/emoji-data}. Using a CDN
	 * for these is not a bad idea.
	 * 
	 * @memberof emoji
	 * @type {
	 */
	emoji.img_sets = {
		'apple' : {
			'path' : 'img/img-apple-64/',
			'sheet' : 'img/sheet_apple_64.png',
			'mask' : 1
		}
	};
	...
	/**
	 * If the platform supports native emoticons, use those instead of the
	 * fallbacks.
	 * 
	 * @memberof emoji
	 * @type bool
	...
	/**
	 * Set to true to use CSS sprites instead of individual images on platforms
	 * that support it.
	 * 
	 * @memberof emoji
	 * @type bool
	 ...
	 * @memberof emoji
	 * @param {string}
	 *            str A string potentially containing ascii emoticons (ie. `:)`)
	 * 
	 * @returns {string} A new string with all emoticons in `str` replaced by a
	 *          representatation that's supported by the current environtment.
	 */
	 ...
	 *            str A string potentially containing ascii emoticons (ie. `:)`)
	 * 
	 * @returns {string} A new string with all emoticons in `str` replaced by
	 *          their colon string representations (ie. `:smile:`)
	 */
	 ...
	 * @memberof emoji
	 * @param {string}
	 *            str A string potentially containing colon string
		...
		return str.replace(emoji.rx_colons, function(m) {
			...
			if (idx.indexOf('::skin-tone-') > -1) {
				...
				if (val) {
					...
					return ':' + idx + ':'
							+ emoji.replacement(skin_val, skin_idx, ':');
				}
			} else {
				var val = emoji.map.colons[idx];
	 ...
	 * @memberof emoji
	 * @param {string}
	 *            str A string potentially containing unified unicode emoticons.
	 *            (ie. 😄)
	 * 
	...
	emoji.replacement = function(idx, actual, wrapper, variation) {
		...
		// for emoji with variation modifiers, set `etxra` to the standalone
		// output for the
		// modifier (used if we can't combine the glyph) and set variation_idx
		// to key of the
		// variation modifier (used below)
		var extra = '';
		var variation_idx = 0;
		if (typeof variation === 'object') {
			extra = emoji.replacement(variation.idx, variation.actual,
					variation.wrapper);
		...
		if (emoji.colons_mode)
			return ':' + emoji.data[idx][3][0] + ':' + extra;
		var text_name = (actual) ? wrapper + actual + wrapper
				: emoji.data[idx][8] || wrapper + emoji.data[idx][3][0]
						+ wrapper;
		if (emoji.text_mode)
			return text_name + extra;

		// native modes next.
		// for variations selectors, we just need to output them raw, which
		// `extra` will contain.
		emoji.init_env();
		if (emoji.replace_mode == 'unified' && emoji.allow_native
				&& emoji.data[idx][0][0])
			return emoji.data[idx][0][0] + extra;
		if (emoji.replace_mode == 'softbank' && emoji.allow_native
				&& emoji.data[idx][1])
			return emoji.data[idx][1] + extra;
		if (emoji.replace_mode == 'google' && emoji.allow_native
				&& emoji.data[idx][2])
			return emoji.data[idx][2] + extra;

		// finally deal with image modes.
		// variation selectors are more complex here - if the image set and
		// particular emoji supports variations, then
		// use the variation image. otherwise, return it as a separate image
		// (already calculated in `extra`).
		// first we set up the params we'll use if we can't use a variation.
		var img = emoji.data[idx][7] || emoji.img_sets[emoji.img_set].path
				+ idx + '.png';
		var title = emoji.include_title ? ' title="'
				+ (actual || emoji.data[idx][3][0]) + '"' : '';
		var text = emoji.include_text ? wrapper
				+ (actual || emoji.data[idx][3][0]) + wrapper : '';
		var px = emoji.data[idx][4];
		var py = emoji.data[idx][5];

		// now we'll see if we can use a varition. if we can, we can override
		// the params above and blank
		...
		if (variation_idx && emoji.variations_data[variation_idx]
				&& emoji.variations_data[variation_idx][2]
				&& !emoji.data[idx][9]) {
			if (emoji.variations_data[variation_idx][2]
					& emoji.img_sets[emoji.img_set].mask) {
				img = emoji.img_sets[emoji.img_set].path + variation_idx
						+ '.png';
		...
		if (emoji.supports_css) {
			if (emoji.use_sheet && px != null && py != null) {
				...
				var style = 'background: url('
						...
						+ ');background-position:' + (mul * px) + '% '
						+ (mul * py) + '%;background-size:' + emoji.sheet_size
						+ '00%';
				...
				return '<span class="emoji emoji-sizer" style="background-image:url('
						+ img + ')"' + title + '>' + text + '</span>' + extra;
			}
		}
	...
	emoji.init_unified = function() {
		...
		emoji.rx_unified = new RegExp('(' + a.join('|')
				+ ')(\uD83C[\uDFFB-\uDFFF])?', "g");
	};
	...
	emoji.show_data = {
		...
		"1f60c" : [ [ "\uD83D\uDE0C" ], "\uE40A", "\uDBB8\uDF3E",
				[ "relieved" ], 26, 5, 15, 0 ],
		"1f60d" : [ [ "\uD83D\uDE0D" ], "\uE106", "\uDBB8\uDF27",
				[ "heart_eyes" ], 26, 6, 15, 0 ],
		"1f60e" : [ [ "\uD83D\uDE0E" ], "", "", [ "sunglasses" ], 26, 7, 15, 0 ],
		"1f60f" : [ [ "\uD83D\uDE0F" ], "\uE402", "\uDBB8\uDF43", [ "smirk" ],
				26, 8, 15, 0 ],
		"1f610" : [ [ "\uD83D\uDE10" ], "", "", [ "neutral_face" ], 26, 9, 15,
				0 ],
		...
		"1f616" : [ [ "\uD83D\uDE16" ], "\uE407", "\uDBB8\uDF3F",
				[ "confounded" ], 26, 15, 15, 0 ],
		"1f617" : [ [ "\uD83D\uDE17" ], "", "", [ "kissing" ], 26, 16, 15, 0 ],
		"1f618" : [ [ "\uD83D\uDE18" ], "\uE418", "\uDBB8\uDF2C",
				[ "kissing_heart" ], 26, 17, 15, 0 ],
		"1f619" : [ [ "\uD83D\uDE19" ], "", "", [ "kissing_smiling_eyes" ], 26,
				18, 15, 0 ],
		"1f61a" : [ [ "\uD83D\uDE1A" ], "\uE417", "\uDBB8\uDF2D",
				[ "kissing_closed_eyes" ], 26, 19, 15, 0 ],
		"1f61b" : [ [ "\uD83D\uDE1B" ], "", "", [ "stuck_out_tongue" ], 26, 20,
				15, 0, ":p" ],
		"1f61c" : [ [ "\uD83D\uDE1C" ], "\uE105", "\uDBB8\uDF29",
				[ "stuck_out_tongue_winking_eye" ], 26, 21, 15, 0, ";p" ],
		"1f61d" : [ [ "\uD83D\uDE1D" ], "\uE409", "\uDBB8\uDF2A",
				[ "stuck_out_tongue_closed_eyes" ], 26, 22, 15, 0 ],
		"1f61e" : [ [ "\uD83D\uDE1E" ], "\uE058", "\uDBB8\uDF23",
				[ "disappointed" ], 26, 23, 15, 0, ":(" ],
		...
		"1f622" : [ [ "\uD83D\uDE22" ], "\uE413", "\uDBB8\uDF39", [ "cry" ],
				26, 27, 15, 0, ":'(" ],
		"1f623" : [ [ "\uD83D\uDE23" ], "\uE406", "\uDBB8\uDF3C",
				[ "persevere" ], 26, 28, 15, 0 ],
		"1f624" : [ [ "\uD83D\uDE24" ], "\uE404", "\uDBB8\uDF28",
				[ "triumph" ], 26, 29, 15, 0 ],
		"1f625" : [ [ "\uD83D\uDE25" ], "\uE401", "\uDBB8\uDF45",
				[ "disappointed_relieved" ], 26, 30, 15, 0 ],
		...
		"1f630" : [ [ "\uD83D\uDE30" ], "\uE40F", "\uDBB8\uDF25",
				[ "cold_sweat" ], 27, 6, 15, 0 ],
		"1f631" : [ [ "\uD83D\uDE31" ], "\uE107", "\uDBB8\uDF41", [ "scream" ],
				27, 7, 15, 0 ],
		"1f632" : [ [ "\uD83D\uDE32" ], "\uE410", "\uDBB8\uDF22",
				[ "astonished" ], 27, 8, 15, 0 ],
		"1f633" : [ [ "\uD83D\uDE33" ], "\uE40D", "\uDBB8\uDF2F",
				[ "flushed" ], 27, 9, 15, 0 ],
		"1f634" : [ [ "\uD83D\uDE34" ], "", "", [ "sleeping" ], 27, 10, 15, 0 ],
		"1f635" : [ [ "\uD83D\uDE35" ], "\uE406", "\uDBB8\uDF24",
				[ "dizzy_face" ], 27, 11, 15, 0 ],
	...
	/*
	 * emoji.include_title = true; emoji.allow_native = false;
	 */
	 ...
	 * 'http://cdn.example.com/emoji/sheet_google_64.png';
	 */
