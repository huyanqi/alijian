...
<script type="text/javascript">
	...
	function getAllUser(){
		$.AMUI.progress.start();
		var path = "<%=basePath%>getUsers";
		var data = {"pageSize":20,"type":1,"status":1};
		$.ajax({
			type : 'POST',
			data : data,
			url : path,
			success : function(result) {
				...
				if(result.result == "ok"){
					...
					$.each(result.data, function(n, value) {
						$("#list_content").append("<tr><td>"+value.name+"</td><td>"+value.mobile+"</td><td>"+value.address+"</td><td><div class='text-right'><a class='btn btn-success btn-mini' href='javascript:pass("+value.id+")'> <i class='icon-edit-sign'></i></a></div></td></tr>");
					});
				}else{
					$("#nodata").show();
					$("#thread").hide();
